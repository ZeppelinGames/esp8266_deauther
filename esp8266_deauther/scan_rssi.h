...
void startRSSI(const rssi_scan_settings_t& settings) {
  ...
  { // Output
    ...
    debugF("Channel time:  ");
    if (rssi_data.settings.ch_time > 0) {
      debugln(strh::time(rssi_data.settings.ch_time));
    } else {
      debuglnF("-");
    }
...
void update_rssi_scan() {
  if (rssi_data.enabled) {
    ...
    if (current_time - rssi_data.last_update_time >= rssi_data.settings.update_time) {
      ...
      if (rssi_buffer.rssi > -35) {
        debugF("[======]");
      }
      else if (rssi_buffer.rssi > -45) {
        debugF("[===== ]");
      }
      else if (rssi_buffer.rssi > -55) {
        debugF("[====  ]");
      }
      else if (rssi_buffer.rssi > -65) {
        debugF("[===   ]");
      }
      else if (rssi_buffer.rssi > -75) {
        debugF("[==    ]");
      }
      else if (rssi_buffer.rssi > -85) {
        debugF("[=     ]");
      }
      else {
        debugF("[      ]");
