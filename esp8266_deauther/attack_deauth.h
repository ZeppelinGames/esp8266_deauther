...
void startDeauth(const deauth_attack_settings_t& settings) {
  ...
  { // Output
    ...
    debugF("Timeout:        ");
    if (deauth_data.settings.timeout > 0) {
      debugln(strh::time(deauth_data.settings.timeout));
    }
    else  {
      debugln('-');
    }
    ...
    debugF("Max. packets:   ");
    if (deauth_data.settings.max_pkts > 0) {
      debugln(deauth_data.settings.max_pkts);
    }
    else {
      debugln('-');
    }
