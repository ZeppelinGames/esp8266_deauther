...
void startAuth(const auth_scan_settings_t& settings) {
  ...
  { // Output
    ...
    debugF("Scan time:     ");
    if (auth_data.settings.timeout > 0) {
      debugln(strh::time(auth_data.settings.timeout));
    } else {
      debuglnF("-");
    }
    ...
    debugF("Channel time:  ");
    if (auth_data.settings.ch_time > 0) {
      debugln(strh::time(auth_data.settings.ch_time));
    } else {
      debuglnF("-");
    }
